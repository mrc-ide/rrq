...
This controller uses an "identifier" (here, `id` is `rrq:babc4f67`) which can be anything you want but acts like a folder within the Redis server, distinguishing your queue from any others hosted on the same server.
...
t
#> [1] "5937a54f2c87e51b4e69e4a6024b2653"
```
...
#> ℹ Spawning 2 workers with prefix 'pokeable_auklet'
```
...
```r
rrq_worker_status()
#> astronomical_izuthrush_1      damaging_platypus_1
#>                   "IDLE"                   "BUSY"
```
...
```r
dir(path)
#> [1] "45d4ac0f45efd78b5333a07e5aef0fa1"
```
...
```r
w <- rrq_worker_spawn(1)
#> ℹ Spawning 1 worker with prefix 'taxidermic_americangoldfinch'
```
