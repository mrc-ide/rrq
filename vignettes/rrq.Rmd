...
This controller uses an "identifier" (here, `id` is `rrq:1abf7df2`) which can be anything you want but acts like a folder within the Redis server, distinguishing your queue from any others hosted on the same server.
...
t <- obj$enqueue(1 + 1)
t
#> [1] "c49670345fef42abe4548d6ae61c39cd"
```
...
#> Spawning 2 workers with prefix evolutional_gnatcatcher
```
...
```r
obj$task_status(t)
#> 9832494930de0252b578b97870421778
#>                        "RUNNING"
```
...
#> 1 evolutional_gnatcatcher_1    NA 1683111446  MESSAGE REFRESH
#> 2 evolutional_gnatcatcher_2    NA 1683111446  MESSAGE REFRESH
#> 3 evolutional_gnatcatcher_1    NA 1683111446    ENVIR     new
#> 4 evolutional_gnatcatcher_2    NA 1683111446    ENVIR     new
#> 5 evolutional_gnatcatcher_1    NA 1683111446    ENVIR  create
#> 6 evolutional_gnatcatcher_2    NA 1683111446    ENVIR  create
#> 7 evolutional_gnatcatcher_1    NA 1683111446 RESPONSE REFRESH
#> 8 evolutional_gnatcatcher_2    NA 1683111446 RESPONSE REFRESH
```
...
```r
obj$task_status(id)
#> e988946b8797020a06bad7d6b9f9d21b
#>                        "PENDING"
```
...
#> ea87cdf68fb811a0e3a74b7f0157c9b0 413a438af5761b45cb1557aad9629328
#>                       "DEFERRED"                       "DEFERRED"
#> 7f6c7a84ff320f4b19c9ed8933bb28a0 4c038ee9e9967452c0b964553d4a846e
#>                       "DEFERRED"                       "DEFERRED"
#> 62539d40abdf011c281ff8ad4afc7ee5
#>                       "DEFERRED"
obj$queue_list()
#> [1] "e988946b8797020a06bad7d6b9f9d21b"
```
...
#>                        "PENDING"
obj$queue_list()
#> [1] "62539d40abdf011c281ff8ad4afc7ee5" "4c038ee9e9967452c0b964553d4a846e"
#> [3] "ea87cdf68fb811a0e3a74b7f0157c9b0" "413a438af5761b45cb1557aad9629328"
#> [5] "7f6c7a84ff320f4b19c9ed8933bb28a0"
```
...
#> Spawning 1 worker with prefix culm_alpinegoat
w_all <- rrq::rrq_worker_spawn(obj, name_config = "all")
#> Spawning 1 worker with prefix bemused_shark
```
...
```r
obj$worker_status()
#> culm_alpinegoat_1   bemused_shark_1
#>            "IDLE"            "BUSY"
```
...
#> Spawning 1 worker with prefix woollen_zanzibardaygecko
```
...
```r
dir(path)
#> [1] "fbb96d45922c6b023b0a67c68359dff6"
```
...
```r
key_alive
#> [1] "rrq:8132201a:worker:alive:c7033fe15971b4f9e0cb39162b1b21cd"
```

How you then start the workers is up to you; you might start them at the command line with

```
rrq_worker --worker-id cluster_1 rrq:8132201a
rrq_worker --worker-id cluster_2 rrq:8132201a
```
...
```r
w <- rrq::rrq_worker_spawn(obj, 1)
#> Spawning 1 worker with prefix grilled_rockrat
```
...
```
#> [2023-05-03 11:57:30] HEARTBEAT rrq:13d01f85:worker:grilled_rockrat_1:heartbeat
#> [2023-05-03 11:57:30] HEARTBEAT OK
#> [2023-05-03 11:57:30] ALIVE
#> [2023-05-03 11:57:30] ENVIR new
#> [2023-05-03 11:57:30] QUEUE default
#>                                  __
#>                 ______________ _/ /
...
#>     username:      rfitzjoh
#>     queue:         rrq:13d01f85:queue:default
#>     wd:            /home/rfitzjoh/Documents/src/rrq/vignettes_src
#>     pid:           1040489
#>     redis_host:    127.0.0.1
#>     redis_port:    6379
#>     heartbeat_key: rrq:13d01f85:worker:grilled_rockrat_1:heartbeat
```
...
```r
obj$task_status(t)
#> 7b2d11d5c93387c88fe9b49288f40601
#>                        "RUNNING"
```
