...
This controller uses an "identifier" (here, `id` is `rrq:6d26a57a`) which can be anything you want but acts like a folder within the Redis server, distinguishing your queue from any others hosted on the same server.
...
t <- obj$enqueue(1 + 1)
t
#> [1] "5f9e7dbcf566c75c92c4951db6b293e9"
```
...
#> Spawning 2 workers with prefix brashy_andalusianhorse
```
...
```r
obj$task_status(t)
#> f061b413987e96869918dd81882be7d3
#>                        "RUNNING"
```
...
#> 1 brashy_andalusianhorse_1    NA 1682519352  MESSAGE REFRESH
#> 2 brashy_andalusianhorse_2    NA 1682519352  MESSAGE REFRESH
#> 3 brashy_andalusianhorse_1    NA 1682519352    ENVIR     new
#> 4 brashy_andalusianhorse_2    NA 1682519352    ENVIR     new
#> 5 brashy_andalusianhorse_1    NA 1682519352    ENVIR  create
#> 6 brashy_andalusianhorse_1    NA 1682519352 RESPONSE REFRESH
#> 7 brashy_andalusianhorse_2    NA 1682519352    ENVIR  create
#> 8 brashy_andalusianhorse_2    NA 1682519352 RESPONSE REFRESH
```
...
```r
obj$task_status(id)
#> 6a329844cec32abc09f8fa8fc8cbc6aa
#>                        "PENDING"
```
...
#> 8da02e9ffefc6976e0cb6d2bfd53cc7a 67abf6f250e9c2a7e37d2252c44ab008
#>                       "DEFERRED"                       "DEFERRED"
#> 108ab77583d8450182e50276cd00e230 f2ef0ad24ac95aea5ac990b591b0ad9f
#>                       "DEFERRED"                       "DEFERRED"
#> 97b66d41c58fed07b6f0af6deb9782c8
#>                       "DEFERRED"
obj$queue_list()
#> [1] "6a329844cec32abc09f8fa8fc8cbc6aa"
```
...
#>                        "PENDING"
obj$queue_list()
#> [1] "97b66d41c58fed07b6f0af6deb9782c8" "67abf6f250e9c2a7e37d2252c44ab008"
#> [3] "8da02e9ffefc6976e0cb6d2bfd53cc7a" "108ab77583d8450182e50276cd00e230"
#> [5] "f2ef0ad24ac95aea5ac990b591b0ad9f"
```
...
```r
w_short <- rrq::rrq_worker_spawn(obj, worker_config = "short")
#> Spawning 1 worker with prefix arthrodial_whippet
w_all <- rrq::rrq_worker_spawn(obj, worker_config = "all")
#> Spawning 1 worker with prefix shrinkable_whitepelican
```
...
```r
obj$worker_status()
#>      arthrodial_whippet_1 shrinkable_whitepelican_1
#>                    "IDLE"                    "BUSY"
```
...
#> Spawning 1 worker with prefix nonchalant_narwhale
```
...
```r
dir(path)
#> [1] "7e27e8c18af698082705267a4716f068"
```
...
If you have submitted workers via a task scheduler, you might want to block and wait for them to become available. You can do this using the `rrq::rrq_worker_expect` and `rrq::rrq_worker_wait` functions.
...
```r
key_alive
#> [1] "rrq:fbc78e7f:worker:alive:e530b6af37bf7fd5e21fe2cd9d4ad7a0"
```

How you then start the workers is up to you; you might start them at the command line with

```
rrq_worker --worker-id cluster_1 rrq:fbc78e7f
rrq_worker --worker-id cluster_2 rrq:fbc78e7f
```
...
```r
w <- rrq::rrq_worker_spawn(obj, 1)
#> Spawning 1 worker with prefix essential_hochstettersfrog
```
...
```
#> [2023-04-26 15:29:16] HEARTBEAT rrq:774aa0f3:worker:essential_hochstettersfrog_1:heartbeat
#> [2023-04-26 15:29:16] HEARTBEAT OK
#> [2023-04-26 15:29:16] ALIVE
#> [2023-04-26 15:29:16] ENVIR new
#> [2023-04-26 15:29:16] QUEUE default
#>                                  __
#>                 ______________ _/ /
...
#> /_____/                   /_/     /_____/
#>     worker:        essential_hochstettersfrog_1
#>     rrq_version:   0.6.21
#>     platform:      x86_64-pc-linux-gnu (64-bit)
#>     running:       Ubuntu 20.04.5 LTS
#>     hostname:      wpia-dide300
#>     username:      rfitzjoh
#>     queue:         rrq:774aa0f3:queue:default
#>     wd:            /home/rfitzjoh/Documents/src/rrq/vignettes_src
#>     pid:           358742
#>     redis_host:    127.0.0.1
#>     redis_port:    6379
#>     heartbeat_key: rrq:774aa0f3:worker:essential_hochstettersfrog_1:heartbeat
```
...
```r
obj$task_status(t)
#> 672fb2452d684f8b00378e5890111aea
#>                        "RUNNING"
```
