...
This controller uses an "identifier" (here, `id` is `rrq:fbd608cf`) which can be anything you want but acts like a folder within the Redis server, distinguishing your queue from any others hosted on the same server.
...
t
#> [1] "f5c8aee4beed505b4e8cb895d26e246f"
```
...
```r
w <- rrq_worker_spawn2(2)
#> ℹ Spawning 2 workers with prefix 'fleece_honeybadger'
```
...
Above, we slept for a few seconds in order for the task to become finished. However, this is an extremely common operation, so `rrq` provides a function `rrq_task_wait()` which will will wait until a task finishes, then returns a logical indicating if the task succeeded or not, after which you can fetch the result with `rrq_task_result()`.
...
#> 1 fleece_honeybadger_1    NA 1713524935  MESSAGE REFRESH
#> 2 fleece_honeybadger_2    NA 1713524935  MESSAGE REFRESH
#> 3 fleece_honeybadger_1    NA 1713524935    ENVIR     new
#> 4 fleece_honeybadger_2    NA 1713524935    ENVIR     new
#> 5 fleece_honeybadger_1    NA 1713524935    ENVIR  create
#> 6 fleece_honeybadger_1    NA 1713524935 RESPONSE REFRESH
#> 7 fleece_honeybadger_2    NA 1713524935    ENVIR  create
#> 8 fleece_honeybadger_2    NA 1713524935 RESPONSE REFRESH
```
...
```r
obj <- rrq_controller2(paste0("rrq:", ids::random_id(bytes = 4)))
rrq_default_controller_set(obj)
...
#> [1] "DEFERRED"
rrq_queue_list()
#> [1] "ed949f0adf14c94340feb180b9ec5b10"
```
...
#> [1] "PENDING"
rrq_queue_list()
#> [1] "73c7f521a87eff53d2aa0b732ebeb78f"
```
...
```r
w_short <- rrq_worker_spawn2(name_config = "short")
#> ℹ Spawning 1 worker with prefix 'puffy_riograndeescuerzo'
w_all <- rrq_worker_spawn2(name_config = "all")
#> ℹ Spawning 1 worker with prefix 'nondestructive_rainbowlorikeet'
```
...
```r
rrq_worker_status()
#>        puffy_riograndeescuerzo_1 nondestructive_rainbowlorikeet_1
#>                           "IDLE"                           "BUSY"
```
...
To configure this you must use `rrq_configure()` *before attaching either a controller or a worker to the queue*. The configuration interface here will change in future, but we will maintain backward compatibility with the current options.
...
#> ℹ Spawning 1 worker with prefix 'semibleached_swan'
```
...
```r
dir(path)
#> [1] "97042aff700694504dd5d956403a82c8"
```
...
* `--config=NAME` allows controlling of the named worker config (set via `rrq_worker_config_save2()`, and allowing changing of timeout, verbosity and queues)
...
```
rrq_worker --worker-id cluster_1 rrq:10388813
rrq_worker --worker-id cluster_2 rrq:10388813
```
...
```r
w <- rrq_worker_spawn2(1)
#> ℹ Spawning 1 worker with prefix 'argumentative_rattail'
```
...
```
#> [2024-04-19 12:09:00.429561] HEARTBEAT rrq:64b5a08d:worker:argumentative_rattail_1:heartbeat
#> [2024-04-19 12:09:00.68014] HEARTBEAT OK
#> [2024-04-19 12:09:00.700099] ALIVE
#> [2024-04-19 12:09:00.700814] ENVIR new
#> [2024-04-19 12:09:00.70176] QUEUE default
#>                                  __
#>                 ______________ _/ /
...
#> /_____/                   /_/     /_____/
#>     worker:        argumentative_rattail_1
#>     config:        localhost
#>     rrq_version:   0.7.13
...
#>     username:      rich
#>     queue:         rrq:64b5a08d:queue:default
#>     wd:            /home/rich/Documents/src/rrq/vignettes_src
#>     pid:           77062
#>     redis_host:    127.0.0.1
#>     redis_port:    6379
#>     heartbeat_key: rrq:64b5a08d:worker:argumentative_rattail_1:heartbeat
```
...
#> Lost 1 worker:
#>   - argumentative_rattail_1
#> Orphaning 1 task:
#>   - cbb15e79e934cd6f78ccdb1ef98dccfd
```
