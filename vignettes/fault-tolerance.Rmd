...
w <- rrq_worker_spawn2()
#> ℹ Spawning 1 worker with prefix 'enharmonic_flamingo'
```
...
#>   queue:  rrq:cd0ee15e
#>   task:   e80b16452857e4748960666e194453ab
#>   status: ERROR
...
#>   queue:  rrq:cd0ee15e
#>   task:   3ed3b309f3d397bd254ca416242767ae
#>   status: DIED
#>   * To throw this error, use stop() with it
```
...
```r
w <- rrq_worker_spawn2()
#> ℹ Spawning 1 worker with prefix 'unquenched_yucker'
```
...
#>   - unquenched_yucker_1
#> Orphaning 1 task:
#>   - 3d583f9bf37cc1845025465e7654a46f
```
...
#>   error:  Task not successful: DIED
#>   queue:  rrq:cd0ee15e
#>   task:   3d583f9bf37cc1845025465e7654a46f
#>   status: DIED
#>   * To throw this error, use stop() with it
```
...
w <- rrq::rrq_worker_spawn2()
#> ℹ Spawning 1 worker with prefix 'balmy_arcticwolf'
```
...
#>   queue:  rrq:cd0ee15e
#>   task:   43d629c2be55dadc12dd00b556c8de0b
#>   status: ERROR
#>   * To throw this error, use stop() with it
#>   * This error has a stack trace, use '$trace' to see it
```
...
#>   queue:  rrq:cd0ee15e
#>   task:   d5167b3e3d8b2921b73983917375963e
#>   status: ERROR
#>   * To throw this error, use stop() with it
#>   * This error has a stack trace, use '$trace' to see it
```
...
#> 43d629c2be55dadc12dd00b556c8de0b 1713526498 1713526498 1713526498    NA
#> d5167b3e3d8b2921b73983917375963e 1713526498 1713526498 1713526498    NA
#> e3fa9e3dc1ad5dc3d088268277c0d35c 1713526498 1713526498 1713526498    NA
rrq_task_times(c(t1, t2, t3), follow = FALSE)
#>                                      submit      start   complete      moved
#> 43d629c2be55dadc12dd00b556c8de0b 1713526497 1713526497 1713526498 1713526498
#> d5167b3e3d8b2921b73983917375963e 1713526498 1713526498 1713526498 1713526498
#> e3fa9e3dc1ad5dc3d088268277c0d35c 1713526498 1713526498 1713526498         NA
```
...
You can inspect the chain of tasks by looking at the return values `rrq_task_info()`. In the above example we have a chain of three tasks 43d629... -> d5167b... -> e3fa9e..., which we can discover this way:


```r
rrq_task_info(t1)
#> $id
#> [1] "43d629c2be55dadc12dd00b556c8de0b"
#>
#> $status
#> [1] "MOVED"
#>
...
#>
#> $moved$down
#> [1] "d5167b3e3d8b2921b73983917375963e" "e3fa9e3dc1ad5dc3d088268277c0d35c"
```
...
#>
#> $down
#> [1] "e3fa9e3dc1ad5dc3d088268277c0d35c"
```
...
#> $up
#> [1] "43d629c2be55dadc12dd00b556c8de0b" "d5167b3e3d8b2921b73983917375963e"
#>
#> $down
#> NULL
```
...
```r
obj_nofollow <- rrq_controller2(obj$queue_id, follow = FALSE)
