...
rrq:::wait_status_change(obj$con, rrq:::rrq_keys(obj$queue_id), t,
                         rrq:::TASK_PENDING)
...
obj$message_send_and_wait("EVAL", quote(set.seed(1)), w$id, progress = FALSE,
                          timeout = 10)
```
...
You can inspect the chain of tasks by looking at the return values `$task_info`. In the above example we have a chain of three tasks `r paste0(substr(c(t1, t2, t3), 1, 6), "...", collapse = " -> ")`, which we can discover this way:
