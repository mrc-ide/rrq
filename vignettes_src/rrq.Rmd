...
## Variable lookup and programming

The above code works like "magic" where we use R's lazy evaluation to intercept an expression and send it to a worker to evaluate later. This tends to work very well interactively but you will find yourself wanting more control.

Sometimes it is convenient to build an expression that you want to compute and pass that around, before finally passing it into `enqueue`; in this case you can do:
...
this is then computed as you'd expect, to return `r round(sqrt(10), 3)`:

```{r}
obj$task_wait(t, 10)
```

You can combine this approach with `rlang`'s "bang-bang" operator (see also [this chapter on "quasiquotation"](https://adv-r.hadley.nz/quasiquotation.html):

```{r}
for (x in c(2, 3, 4)) {
  t <- obj$enqueue(sqrt(!!x))
  message(sprintf("sqrt(%d) => %f", x, obj$task_wait(t, 2)))
}
```

There's not much advantage of doing this over the built-in rrq approach though:
...
Finally, you could combine `!!` and `{{`; but do to this you need to use `rlang::inject`:

```{r}
x <- 2
expr <- rlang::inject(quote(sqrt(!!x)))
t <- obj$enqueue({{ expr }})
obj$task_wait(t, 2)
```
...
```{r, include = FALSE}
w <- rrq::rrq_worker$new(obj$queue_id)
w$step(TRUE)
```
...
```{r include = FALSE}
invisible(obj$task_wait(t, timeout = 10))
```
...
```{r, echo = FALSE}
writeLines(obj$worker_process_log(obj$worker_list()))
```
