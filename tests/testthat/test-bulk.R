...
test_that("bulk tasks can be queued with dependency", {
  ...
  expect_equivalent(obj$task_status(t3), "DEFERRED")
  expect_equal(obj$queue_list(), grp$task_ids)
  expect_equal(obj$con$SMEMBERS(key_queue_deferred), list(t3))

  w$step(TRUE)
  obj$task_wait(grp_id_1, 2)
  expect_equivalent(obj$task_status(grp$task_ids), c("COMPLETE", "PENDING"))
  expect_equivalent(obj$task_status(t3), "DEFERRED")
  expect_equal(obj$queue_list(), grp_id_2)
  expect_equal(obj$con$SMEMBERS(key_queue_deferred), list(t3))

  w$step(TRUE)
  obj$task_wait(grp_id_2, 2)
