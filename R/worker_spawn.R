...
## TODO: Better recover here.  Ideally we'd stop any workers that
## *are* running, and provide data from the log files.  Because the
## worker names are often auto-generated, finding out what went wrong
## is pretty hard work.  However, getting the log is also pretty
## tricky; can get that from the context, but that's not passed
## through here.
